(ns audio.loop-test
  "Full tick! cycle, fully offline: audio.murakumo/audio.judge/datalad shell
...
(defn- tmp-dir []
  (let [f (java.io.File/createTempFile "audio-loop-test" "")]
    (.delete f) (.mkdirs f) f))
    ...
    (with-tmp-paths tmp
      (fn []
        (with-redefs [murakumo/submit! fake-submit!
          ...
          (testing "accepted fact carries a :music or :sfx modality (dual-modality actor)"
            (let [facts (ledger/read-all (str tmp "/ledger.edn"))
                  accepted (first (filter #(= :accepted (:t %)) facts))]
              (is (contains? #{:music :sfx} (:modality accepted)))))
