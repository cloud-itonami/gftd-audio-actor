(ns audio.cosci
  ...
  music/SFX candidates (ADR-2607122400 §3), ported-in-shape from
  ...
  slightly-worse asset, it's a disqualified one.
    ...
    Generation  -> audio.generate/round-candidates (closed gene pool, mixes
                   :music and :sfx flavored candidates in the same round)
    Reflection  -> hard gates: job actually :done, artifact fetched,
                   declared format matches, no upstream safety flag
    Ranking     -> Elo on audio.judge scores (pairwise, draw-tolerant)
...
(defn reflect
  ...
  :format \"wav\" :expected-format \"wav\" :safety-flag bool}. Format check
  alone covers both :music and :sfx candidates — murakumo's
  default-out-by-modality maps both to :wav, so no modality-aware branch is
  needed here."
...
(defn rank
  ...
  missing entries when audio.judge/score! failed) -> ranked vector,
...
(defn evolve-round
  "ranked -> {:next-round n+1 :elite-gene {...}} for audio.generate's next
